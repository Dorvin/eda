...
def load_data(location = 'dataset1_parsed.mat', ten=9, inv=False):
    # load eeg data
...
c_learning_rate_0 = 3e-6
c_learning_rate_1 = 1e-4
c_learning_rate_3 = 1e-9
c_learning_rate_6 = 1e-8
# default training epoch is 1500
c_training_epoch = 2000
...
for data_num in (0,):
  for inv in (False,):
    for until in (0, 1, 5):
      ...
      # build nn of rightside(learnable)
      e2_W1 = tf.get_variable("e2_W1", shape=[num_input, 4096], initializer=tf.contrib.layers.xavier_initializer())
      e2_b1 = tf.get_variable("e2_b1", shape=[4096], initializer=tf.contrib.layers.xavier_initializer())
      e2_W2 = tf.get_variable("e2_W2", shape=[4096, 2048], initializer=tf.contrib.layers.xavier_initializer())
      e2_b2 = tf.get_variable("e2_b2", shape=[2048], initializer=tf.contrib.layers.xavier_initializer())
      e2_W3 = tf.get_variable("e2_W3", shape=[2048, 1024], initializer=tf.contrib.layers.xavier_initializer())
      e2_b3 = tf.get_variable("e2_b3", shape=[1024], initializer=tf.contrib.layers.xavier_initializer())
      e2_W4 = tf.get_variable("e2_W4", shape=[1024, 512], initializer=tf.contrib.layers.xavier_initializer())
      e2_b4 = tf.get_variable("e2_b4", shape=[512], initializer=tf.contrib.layers.xavier_initializer())
      e2_W5 = tf.get_variable("e2_W5", shape=[512, 256], initializer=tf.contrib.layers.xavier_initializer())
      e2_b5 = tf.get_variable("e2_b5", shape=[256], initializer=tf.contrib.layers.xavier_initializer())
      encoder2 = tf.nn.tanh(tf.matmul(X, e2_W1) + e2_b1)
      encoder2 = tf.nn.tanh(tf.matmul(encoder2, e2_W2) + e2_b2)
      encoder2 = tf.nn.tanh(tf.matmul(encoder2, e2_W3) + e2_b3)
      encoder2 = tf.nn.tanh(tf.matmul(encoder2, e2_W4) + e2_b4)
      encoder2 = tf.nn.tanh(tf.matmul(encoder2, e2_W5) + e2_b5)

      # concat two nns
      bottleneck = tf.concat([encoder, encoder2], 1)

      # final FC to classify using result of concat
      ...
      c_W2 = tf.get_variable("c_W2", shape=[256, 1], initializer=tf.contrib.layers.xavier_initializer())
      ...
      c_logit = tf.matmul(c_logit, c_W2) + c_b2
      c_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=Y, logits=c_logit))
      # for tensorboard
      tf.summary.scalar('loss', c_loss)

      # use different optimizer for each datasets
      c_optimizer0 = tf.train.AdamOptimizer(c_learning_rate_0).minimize(c_loss, var_list=[e2_W1, e2_b1, e2_W2, e2_b2, e2_W3, e2_b3, e2_W4, e2_b4, e2_W5, e2_b5, c_W, c_b, c_W2, c_b2])
      c_optimizer1 = tf.train.AdamOptimizer(c_learning_rate_1).minimize(c_loss, var_list=[e2_W1, e2_b1, e2_W2, e2_b2, e2_W3, e2_b3, e2_W4, e2_b4, e2_W5, e2_b5, c_W, c_b, c_W2, c_b2])
      c_optimizer3 = tf.train.AdamOptimizer(c_learning_rate_3).minimize(c_loss, var_list=[e2_W1, e2_b1, e2_W2, e2_b2, e2_W3, e2_b3, e2_W4, e2_b4, e2_W5, e2_b5, c_W, c_b, c_W2, c_b2])
      c_optimizer6 = tf.train.AdamOptimizer(c_learning_rate_6).minimize(c_loss, var_list=[e2_W1, e2_b1, e2_W2, e2_b2, e2_W3, e2_b3, e2_W4, e2_b4, e2_W5, e2_b5, c_W, c_b, c_W2, c_b2])
      c_optimizer = None
      ...
      with tf.Session() as sess:
        print(f'----------dataset{data_num} & inv {inv} & until {until}----------')
        ...
        for ten_num in range(0, 10):
          ...
          if until == 1:
            ...
            saver = tf.train.Saver(var_list = [e_W1, e_b1, e_W2, e_b2, e_W3, e_b3, e_W4, e_b4, e_W5, e_b5])
          ...
          # get batch_size and batch_num(which should be 9)
          ...
          train_summary_path = f"./logs/{data_num}_{inv}_until{until}/{ten_num}/train"
          val_summary_path = f"./logs/{data_num}_{inv}_until{until}/{ten_num}/val"
